...
struct stripe_request {
    ...
    /* The stripe's third parity chunk, used by the Stage1 RAIDX extension. */
    struct chunk *r_chunk;

    /* degrade mode*/
    enum stripe_read_degraded_type {
        DEGRADED_D,
        DEGRADED_DP,
        DEGRADED_DD,
        NORMAL,
    ...
    enum stripe_write_degraded_type {
        DEGRADED_WRITE_RMW,
        DEGRADED_WRITE_RCW,
        DEGRADED_WRITE_TBD,
        DEGRADED_WRITE_DIRECT,
        NORMAL_WRITE,
        TOO_MANY_DEGRADED,
    } degraded_type_write;
...
struct raid6_info {
    ...
    unsigned char *gf_const_tbl_arr_r[256];
...
static inline bool
raid6_is_parity_chunk(struct stripe_request *stripe_req, struct chunk *chunk)
{
    return chunk == stripe_req->parity_chunk ||
           chunk == stripe_req->q_chunk ||
           (stripe_req->r_chunk != NULL && chunk == stripe_req->r_chunk);
}

static inline struct chunk *
raid6_next_data_chunk(struct stripe_request *stripe_req, struct chunk *chunk)
{
    do {
        chunk++;
    } while (chunk < stripe_req->chunks + stripe_req->raid_io->raid_bdev->num_base_bdevs &&
             raid6_is_parity_chunk(stripe_req, chunk));

    return chunk;
}

#define FOR_EACH_DATA_CHUNK(req, c) \
	for (c = raid6_next_data_chunk(req, req->chunks-1); \
	     c < req->chunks + req->raid_io->raid_bdev->num_base_bdevs; \
	     c = raid6_next_data_chunk(req, c))
...
raid6_chunk_data_index(struct chunk *chunk)
{
    ...
    for (iter = stripe_req->chunks; iter < chunk; iter++) {
        if (!raid6_is_parity_chunk(stripe_req, iter)) {
            data_idx++;
        }
    }
...
raid6_get_data_chunk(struct stripe_request *stripe_req, uint8_t chunk_data_idx)
{
    ...
    FOR_EACH_CHUNK(stripe_req, chunk) {
        if (raid6_is_parity_chunk(stripe_req, chunk)) {
            continue;
        }
        if (data_idx == chunk_data_idx) {
            return chunk;
        }
        data_idx++;
    }
...
static inline bool
raid6_uses_r_parity(const struct raid_bdev *raid_bdev)
{
    return raid_bdev->module->base_bdevs_max_degraded > 2;
}
...
static void
raid6_r_gen_iovs(struct iovec *iovs_dest, int iovs_dest_cnt, size_t iovs_dest_offset,
...
raid6_r_gen_iovs(struct iovec *iovs_dest, int iovs_dest_cnt, size_t iovs_dest_offset,
                 ...
                 size_t size, struct raid6_info *r6info, uint8_t index)
{
    raid6_gf_mul_iovs(iovs_dest, iovs_dest_cnt, iovs_dest_offset,
                      iovs_src, iovs_src_cnt, iovs_src_offset,
                      size, r6info->gf_const_tbl_arr_r[index]);
}
...
raid6_complete_chunk_request(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
    ...
    if (chunk->latency_breakdown_correlation_id != 0 &&
        ...
        semiraid_lb_emit_host_command(
            stripe_req->raid_io->latency_breakdown_request_id,
            chunk->latency_breakdown_correlation_id,
            chunk->index,
            semiraid_lb_stage_for_operation(
                chunk->request_type == CHUNK_WRITE ? SEMIRAID_LB_OP_WRITE : SEMIRAID_LB_OP_READ),
            chunk->request_type == CHUNK_WRITE ? SEMIRAID_LB_OP_WRITE : SEMIRAID_LB_OP_READ,
            chunk->latency_breakdown_submit_ticks,
            spdk_get_ticks(),
...
raid6_stripe_write_preread_complete_rmw(struct stripe_request *stripe_req)
{
    ...
    struct chunk *p_chunk = stripe_req->parity_chunk;
    struct chunk *q_chunk = stripe_req->q_chunk;
    struct chunk *r_chunk = stripe_req->r_chunk;
    struct raid_bdev *raid_bdev = stripe_req->raid_io->raid_bdev;
    struct raid6_info *r6info = raid_bdev->module_private;
    uint32_t blocklen = raid_bdev->bdev.blocklen;
    ...
    FOR_EACH_DATA_CHUNK(stripe_req, chunk) {
        ...
        raid6_xor_iovs(q_chunk->iovs, q_chunk->iovcnt, dest_offset,
                       ...
                       chunk->req_blocks * blocklen);
        if (r_chunk != NULL) {
            raid6_r_gen_iovs(&iov, 1, 0,
                             chunk->iovs, chunk->iovcnt, 0,
                             chunk->req_blocks * blocklen, r6info, raid6_chunk_data_index(chunk));
            raid6_xor_iovs(r_chunk->iovs, r_chunk->iovcnt, dest_offset,
                           &iov, 1, 0,
                           chunk->req_blocks * blocklen);
        }
...
raid6_stripe_write_preread_complete(struct stripe_request *stripe_req)
{
    ...
    raid6_memset_iovs(q_chunk->iovs, q_chunk->iovcnt, 0);
    if (r_chunk != NULL) {
        raid6_memset_iovs(r_chunk->iovs, r_chunk->iovcnt, 0);
    }
        ...
        if (chunk->preread_blocks > 0) {
            ...
            if (r_chunk != NULL) {
                raid6_r_gen_iovs(&iov, 1, 0,
                                 chunk->iovs, chunk->iovcnt, 0,
                                 chunk->preread_blocks * blocklen, r6info, raid6_chunk_data_index(chunk));
                raid6_xor_iovs(r_chunk->iovs, r_chunk->iovcnt,
                               (chunk->preread_offset - r_chunk->req_offset) * blocklen,
                               &iov, 1, 0,
                               chunk->preread_blocks * blocklen);
            }
        }
        ...
        if (chunk->req_blocks > 0) {
            ...
            if (r_chunk != NULL) {
                raid6_r_gen_iovs(&iov, 1, 0,
                                 ...
                                 chunk->req_blocks * blocklen, r6info, raid6_chunk_data_index(chunk));
                raid6_xor_iovs(r_chunk->iovs, r_chunk->iovcnt,
                               (chunk->req_offset - r_chunk->req_offset) * blocklen,
                               &iov, 1, 0,
                               chunk->req_blocks * blocklen);
            }
        }
...
static void
raid6_stripe_write_preread_complete_degraded(struct stripe_request *stripe_req)
{
    struct chunk *chunk;
    struct chunk *p_chunk = stripe_req->parity_chunk;
    struct chunk *q_chunk = stripe_req->q_chunk;
    ...
    int ret = 0;
    struct chunk *d_chunk_x, *d_chunk_y;
    uint64_t src_offset, len, real_preread_offset, real_preread_blocks;
    uint8_t i;

    if (p_chunk->preread_blocks || q_chunk->preread_blocks) {
        if (p_chunk->preread_blocks && q_chunk->preread_blocks) {//P+Q
            ...
            for (i = 0; i < 2; i++) {
                stripe_req->degraded_chunks[i]->preread_offset = p_chunk->req_offset;
            ...
            void *tmp_buf1 = stripe_req->stripe->chunk_buffers[raid_bdev->num_base_bdevs + 2];
            struct iovec iov_tmp0 = {
                    .iov_base = tmp_buf0,
                    .iov_len = raid_bdev->strip_size * blocklen,
            };
            ...
            raid6_memset_iovs(&iov_tmp0, 1, 0);
            raid6_memset_iovs(&iov_tmp1, 1, 0);

            // Note: construct d_chunk_x
            FOR_EACH_CHUNK(stripe_req, chunk) {
                if (!chunk->is_degraded) {
                    src_offset = (p_chunk->preread_offset - chunk->preread_offset) * blocklen;
                    if (chunk == p_chunk) {
                        raid6_xor_iovs(d_chunk_y->iovs, d_chunk_y->iovcnt, 0,
                                       chunk->iovs, chunk->iovcnt, src_offset,
                                       len);
                    } else if (chunk == q_chunk) {
                        raid6_xor_iovs(&iov_tmp0, 1, 0,
                        ...
                        raid6_q_gen_iovs(&iov_tmp1, 1, 0,
                                         chunk->iovs, chunk->iovcnt, src_offset,
                                         len, r6info, raid6_chunk_data_index(chunk));
                        raid6_xor_iovs(&iov_tmp0, 1, 0,
                                       &iov_tmp1, 1, 0,
                                       len);
                    }
            ...
            raid6_gf_mul_iovs(d_chunk_x->iovs, d_chunk_x->iovcnt, 0,
                              d_chunk_y->iovs, d_chunk_y->iovcnt, 0,
                              len, gf_const_tbl_a);
            raid6_gf_mul_iovs(&iov_tmp1, 1, 0,
                              &iov_tmp0, 1, 0,
                              len, gf_const_tbl_b);
            raid6_xor_iovs(d_chunk_x->iovs, d_chunk_x->iovcnt, 0,
                           &iov_tmp1, 1, 0,
                           len);
            ...
            // Note: construct d_chunk
            FOR_EACH_CHUNK(stripe_req, chunk) {
                if (chunk != stripe_req->degraded_chunks[i] && chunk != p_chunk) {
                    src_offset = (stripe_req->degraded_chunks[i]->preread_offset - chunk->preread_offset) * blocklen;
                    if (chunk == q_chunk) {
                        raid6_xor_iovs(stripe_req->degraded_chunks[i]->iovs, stripe_req->degraded_chunks[i]->iovcnt, 0,
                                       chunk->iovs, chunk->iovcnt, src_offset,
                        ...
                        raid6_q_gen_iovs(&iov1, 1, 0,
                                         chunk->iovs, chunk->iovcnt, src_offset,
                                         stripe_req->degraded_chunks[i]->preread_blocks * blocklen, r6info, raid6_chunk_data_index(chunk));
                        raid6_xor_iovs(stripe_req->degraded_chunks[i]->iovs, stripe_req->degraded_chunks[i]->iovcnt, 0,
                                       &iov1, 1, 0,
                                       stripe_req->degraded_chunks[i]->preread_blocks * blocklen);
                    }
        ...
        FOR_EACH_DATA_CHUNK(stripe_req, chunk) {
            if (stripe_req->first_data_chunk == stripe_req->last_data_chunk) {
                if (chunk->req_blocks) { // Note: don't read request chunks
                    ...
                    real_preread_blocks = 0;
                } else { // Note: read non-request chunks
                    real_preread_offset = p_chunk->req_offset;
                    real_preread_blocks = p_chunk->req_blocks;
                }
            } else {
            ...
            if (real_preread_blocks > 0) {
                ...
                raid6_q_gen_iovs(&iov, 1, 0,
                                 chunk->iovs, chunk->iovcnt,
                                 (real_preread_offset - p_chunk->preread_offset) * blocklen,
                                 real_preread_blocks * blocklen, r6info, raid6_chunk_data_index(chunk));
                raid6_xor_iovs(q_chunk->iovs, q_chunk->iovcnt,
                               (real_preread_offset- q_chunk->req_offset) * blocklen,
                               &iov, 1, 0,
                               real_preread_blocks * blocklen);
            }
...
static void
raid6_check_degraded_write(struct stripe_request *stripe_req)
{
    struct chunk *chunk;
    struct chunk *p_chunk = stripe_req->parity_chunk;
    struct chunk *q_chunk = stripe_req->q_chunk;
    ...
    if (raid6_uses_r_parity(raid_bdev) && total_degraded > 0) {
        stripe_req->degraded_type_write = TOO_MANY_DEGRADED;
        return;
    }
    ...
    if (total_degraded > raid_bdev->module->base_bdevs_max_degraded) {
        ...
        if (p_chunk->is_degraded) {
            if (p_chunk == stripe_req->degraded_chunks[0]) {
                chunk = stripe_req->degraded_chunks[1];
            } else {
                chunk = stripe_req->degraded_chunks[0];
            }
            if (chunk == q_chunk) {
                stripe_req->degraded_type_write = DEGRADED_WRITE_DIRECT; //P+Q
            } else if (chunk->req_blocks > 0) {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RCW; //P+D_w
            } else {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RMW; //P+D_nw
            }
        } else if (q_chunk->is_degraded) {
            if (q_chunk == stripe_req->degraded_chunks[0]) {
                chunk = stripe_req->degraded_chunks[1];
            } else {
                chunk = stripe_req->degraded_chunks[0];
            }
            if (chunk->req_blocks > 0) {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RCW; //Q+D_w
            } else {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RMW; //Q+D_nw
            }
        } else {
            if (stripe_req->degraded_chunks[0]->req_blocks > 0 || stripe_req->degraded_chunks[1]->req_blocks > 0) {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RCW; //D_w+D_w, D_w+D_nw
            } else {
                stripe_req->degraded_type_write = DEGRADED_WRITE_RMW; //D_nw+D_nw
            }
        }
    } else if (total_degraded == 1) {
        if (p_chunk->is_degraded || q_chunk->is_degraded) {
            stripe_req->degraded_type_write = DEGRADED_WRITE_RCW; //P, Q
        } else if (stripe_req->degraded_chunks[0]->req_blocks > 0) {
            stripe_req->degraded_type_write = DEGRADED_WRITE_RCW; //D_w
        } else {
            stripe_req->degraded_type_write = DEGRADED_WRITE_RMW; //D_nw
        }
    } else {
        stripe_req->degraded_type_write = NORMAL_WRITE;
    } 
}

static void
raid6_degraded_write(struct stripe_request *stripe_req)
{
    struct raid_bdev *raid_bdev = stripe_req->raid_io->raid_bdev;
    struct chunk *p_chunk = stripe_req->parity_chunk;
    struct chunk *q_chunk = stripe_req->q_chunk;
    ...
    if (stripe_req->first_data_chunk == stripe_req->last_data_chunk) {
        ...
        if (r_chunk != NULL) {
            r_chunk->req_offset = stripe_req->first_data_chunk->req_offset;
            r_chunk->req_blocks = stripe_req->first_data_chunk->req_blocks;
        }
    } else {
        ...
        if (r_chunk != NULL) {
            r_chunk->req_offset = 0;
            r_chunk->req_blocks = raid_bdev->strip_size;
        }
    }
    ...
    if (stripe_req->degraded_type_write == DEGRADED_WRITE_RMW) {
        stripe_req->chunk_requests_complete_cb = raid6_stripe_write_preread_complete_rmw;
    } else if (stripe_req->degraded_type_write == DEGRADED_WRITE_RCW) {
        stripe_req->chunk_requests_complete_cb = raid6_stripe_write_preread_complete_degraded;
    } else {
    ...
    FOR_EACH_CHUNK(stripe_req, chunk) {
        ...
        if (stripe_req->chunk_requests_complete_cb == raid6_stripe_write_preread_complete_rmw) {
            chunk->preread_offset = chunk->req_offset;
            chunk->preread_blocks = chunk->req_blocks;           
        } else {
            if (stripe_req->first_data_chunk == stripe_req->last_data_chunk) {
                if (stripe_req->degraded_chunks[1] == NULL || p_chunk->is_degraded || q_chunk->is_degraded)  {
                    if (chunk->req_blocks) {
                        chunk->preread_offset = 0;
                        chunk->preread_blocks = 0;
                    } else {
                ...
                } else { // TODO need more work upon completion
                    chunk->preread_offset = p_chunk->req_offset;
                    chunk->preread_blocks = p_chunk->req_blocks;
                }
            } else {
                if (stripe_req->degraded_chunks[1] == NULL) { //single degraded drive
                    if (stripe_req->degraded_chunks[0] == p_chunk ||
                        stripe_req->degraded_chunks[0] == q_chunk ||
                        (stripe_req->degraded_chunks[0]->req_offset == 0 && 
                        stripe_req->degraded_chunks[0]->req_blocks == raid_bdev->strip_size)) {
                        ...
                        if (chunk == q_chunk) {
                            chunk->preread_offset = 0;
                            chunk->preread_blocks = 0;
                        } else if (chunk == p_chunk) {
                            if (stripe_req->degraded_chunks[0]->req_offset) {
                                chunk->preread_offset = 0;
                                chunk->preread_blocks = stripe_req->degraded_chunks[0]->req_offset;
                            } else {
                                chunk->preread_offset = stripe_req->degraded_chunks[0]->req_blocks;
                                chunk->preread_blocks = raid_bdev->strip_size - stripe_req->degraded_chunks[0]->req_blocks;
                            }
                    ...
                    for (uint8_t i = 0; i < 2; i++) {
                        if (stripe_req->degraded_chunks[i] != p_chunk && stripe_req->degraded_chunks[i] != q_chunk) {
                            degraded_data_chunks[count] = stripe_req->degraded_chunks[i];
                            count++;
                        }
                    }
                    if (count == 1) { //one is data drive
                        if (degraded_data_chunks[0]->req_offset == 0 && degraded_data_chunks[0]->req_blocks == raid_bdev->strip_size) {
                            if (chunk == p_chunk || chunk == q_chunk) {
                                chunk->preread_offset = 0;
                                chunk->preread_blocks = 0;
                            } else if (chunk->req_offset) {
                            ...
                            if (chunk == p_chunk || chunk == q_chunk) {
                                if (degraded_data_chunks[0]->req_offset) {
                                    chunk->preread_offset = 0;
                                    chunk->preread_blocks = degraded_data_chunks[0]->req_offset;
                                } else {
                                    chunk->preread_offset = degraded_data_chunks[0]->req_blocks;
                                    chunk->preread_blocks = raid_bdev->strip_size - degraded_data_chunks[0]->req_blocks;
                                }
                            } else if (chunk == stripe_req->first_data_chunk || chunk == stripe_req->last_data_chunk || !chunk->req_blocks) {
                                chunk->preread_offset = 0;
                                chunk->preread_blocks = raid_bdev->strip_size;
                    ...
                    } else { //both are data drives
                        if (degraded_data_chunks[0]->req_offset == 0 && degraded_data_chunks[0]->req_blocks == raid_bdev->strip_size &&
                        degraded_data_chunks[1]->req_offset == 0 && degraded_data_chunks[1]->req_blocks == raid_bdev->strip_size) {
                            if (chunk == p_chunk || chunk == q_chunk) {
                                chunk->preread_offset = 0;
                                chunk->preread_blocks = 0;
                            } else if (chunk->req_offset) {
...
raid6_stripe_write(struct stripe_request *stripe_req)
{
    ...
    raid6_check_degraded_write(stripe_req);
    
    if (stripe_req->degraded_type_write == TOO_MANY_DEGRADED) {
        raid6_abort_stripe_request(stripe_req, SPDK_BDEV_IO_STATUS_FAILED);
        return;
    }

    if (stripe_req->degraded_type_write != NORMAL_WRITE) {
        raid6_degraded_write(stripe_req);
        return;
    }
    ...
    FOR_EACH_CHUNK(stripe_req, chunk) {
        if (preread_balance > 0) { // Note: for RMW, request chunks are the same as the preread chunks
            ...
            if (raid6_is_parity_chunk(stripe_req, chunk)) { // Note: for reconstruction write, no need to read parity
                chunk->preread_offset = 0;
                chunk->preread_blocks = 0;
        ...
        if (chunk->preread_blocks || raid6_is_parity_chunk(stripe_req, chunk)) {
            size_t len;
...
raid6_stripe_read(struct stripe_request *stripe_req)
{
    ...
    enum stripe_read_degraded_type degraded_type;
                    ...
                    uint64_t stripe_offset, uint64_t blocks, uint64_t iov_offset)
{
    ...
    uint8_t r_idx = (p_idx + 2) % raid_bdev->num_base_bdevs;
    stripe_req->parity_chunk = &stripe_req->chunks[p_idx];
    stripe_req->q_chunk = &stripe_req->chunks[q_idx];
    stripe_req->r_chunk = raid6_uses_r_parity(raid_bdev) ? &stripe_req->chunks[r_idx] : NULL;
...
static void
raid6_complete_chunk_request_read(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg);
...
raid6_complete_chunk_request_read(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg)
{
...
#if defined(SEMIRAID_ENABLE_LATENCY_BREAKDOWN)
    if (iov_w->latency_breakdown_correlation_id != 0 &&
        iov_w->latency_breakdown_submit_ticks != 0) {
        semiraid_lb_emit_host_command(
            raid_io->latency_breakdown_request_id,
            iov_w->latency_breakdown_correlation_id,
            iov_w->latency_breakdown_target_id,
            SEMIRAID_LB_STAGE_READ,
            SEMIRAID_LB_OP_READ,
            iov_w->latency_breakdown_submit_ticks,
            spdk_get_ticks(),
            success ? SEMIRAID_LB_FLAG_SUCCESS : 0);
    ...
    TAILQ_INSERT_TAIL(&r6ch->iov_w_queue, iov_w, link);
}

static int
raid6_map_iov(struct iovec *iovs, const struct iovec *iov, int iovcnt,
              uint64_t offset, uint64_t len)
{
    int i;
    size_t off = 0;
...
raid6_handle_read(struct raid_bdev_io *raid_io, uint64_t stripe_index, uint64_t stripe_offset, uint64_t blocks)
{
    ...
    uint8_t p_chunk_idx = raid_bdev->num_base_bdevs - 1 - (stripe_index % raid_bdev->num_base_bdevs);
    uint8_t q_chunk_idx = (p_chunk_idx + 1) % raid_bdev->num_base_bdevs;
    ...
    for (uint8_t i = 0; i < raid_bdev->num_base_bdevs; i++) {
        if (i == p_chunk_idx || i == q_chunk_idx || i < first_chunk_idx || i > last_chunk_idx) {
            ...
            iov_w = TAILQ_FIRST(&r6ch->iov_w_queue);
            TAILQ_REMOVE(&r6ch->iov_w_queue, iov_w, link);
            iov_w->num_blocks = chunk_req_blocks;
            iov_w->raid_io = raid_io;

            chunk_iovcnt = raid6_map_iov(iov_w->iovs, iov, iovcnt, iov_offset, chunk_len);
...
#else
            spdk_bdev_readv_blocks(base_info->desc, base_ch, iov_w->iovs,
                                   chunk_iovcnt, base_offset_blocks,
                                   chunk_req_blocks,
                                   raid6_complete_chunk_request_read, iov_w);
#endif
        }
    }
...
raid6_start(struct raid_bdev *raid_bdev)
{
    ...
    for (int i = 0; i < 256; i++) {
        ...
        r6info->gf_const_tbl_arr_r[i] = calloc(32, sizeof(unsigned char));
        r6info->gf_const_tbl_arr_a[i] = calloc(32, sizeof(unsigned char));
        if (!r6info->gf_const_tbl_arr[i] || !r6info->gf_const_tbl_arr_r[i] ||
            !r6info->gf_const_tbl_arr_a[i]) {
    ...
    for (int i = 0; i < 256; i++) {
        ...
        gf_vect_mul_init(c_r, r6info->gf_const_tbl_arr_r[i]);
...
raid6_io_channel_resource_deinit(void *resource)
{
    ...
    struct iov_wrapper *iov_w;
    while (!TAILQ_EMPTY(&r6ch->iov_w_queue)) {
        iov_w = TAILQ_FIRST(&r6ch->iov_w_queue);
        TAILQ_REMOVE(&r6ch->iov_w_queue, iov_w, link);
        free(iov_w);
    }
